defmodule Mix.Tasks.Saas.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix saas.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      mix saas.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `saas.gen.schema` for more information on
  attributes.
  """
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise "mix saas.gen.context must be invoked from within your *_web application root directory"
    end
    ...
    context
    |> copy_new_files(paths, binding, accountscope)
  ...
  @doc false
  def copy_new_files(%Context{schema: schema} = context, paths, binding, accountscope) do
    if schema.generate?, do: Gen.Schema.copy_new_files(schema, paths, binding)
    inject_schema_access(context, paths, binding, accountscope)
    inject_tests(context, paths, binding, accountscope)
    create_fixture(context, paths, binding, accountscope)

    context
  end

  defp inject_schema_access(%Context{file: file} = context, paths, binding, accountscope) do
    scope_prefix = if accountscope, do: "accountscoped_", else: ""

    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(file, Mix.Phoenix.eval_from(paths, "priv/templates/saas.gen.context/#{scope_prefix}context.ex", binding))
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/saas.gen.context/#{scope_prefix}#{schema_access_template(context)}", binding)
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding, accountscope) do
    scope_prefix = if accountscope, do: "accountscoped_", else: ""

    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(test_file, Mix.Phoenix.eval_from(paths, "priv/templates/saas.gen.context/#{scope_prefix}context_test.exs", binding))
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/saas.gen.context/#{scope_prefix}test_cases.exs", binding)
    |> inject_eex_before_final_end(test_file, binding)
  end

  defp create_fixture(%Context{basename: basename}, paths, binding, accountscope) do
    scope_prefix = if accountscope, do: "accountscoped_", else: ""
    fixture_file = "test/support/fixtures/#{basename}_fixtures.ex"
    Mix.Generator.create_file(fixture_file, Mix.Phoenix.eval_from(paths, "priv/templates/saas.gen.context/#{scope_prefix}fixtures.exs", binding))
  end
  ...
  def print_shell_instructions(%Context{schema: schema} = context) do
    if schema.generate? do
      ...
      Mix.shell().info """
      Remember that you might need to add in the Schema for #{schema.file}

        belongs_to :account, #{inspect context.base_module}.Accounts.Account
      """
    else
      :ok
    end
  ...
  def raise_with_help(msg) do
    ...
    mix saas.gen.html, saas.gen.json, saas.gen.live, and saas.gen.context
    ...
    For example:
        ...
        mix saas.gen.html Accounts User users name:string
        mix saas.gen.json Accounts User users name:string
        mix saas.gen.live Accounts User users name:string
        mix saas.gen.context Accounts User users name:string
